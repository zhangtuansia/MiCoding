...
- 可为两个 AI 分别配置 8 个环形动作；电源键单击打开 Codex，双击打开 Claude，并自动聚焦提示词输入框
- 语音键可直接启动或结束 Typeless 听写；遥控器会在 HID 层发送真实的 `F20`，避开 `fn` 被输入法或 macOS 抢占的问题
- 首次使用时，在 Typeless“设置 → 键盘快捷键 → 语音输入”中保留原有快捷键，并通过“添加另一个”加入 `F20`
...
- 遥控器麦克风的原始 HID 音频报告已接入诊断链路；在完成具体固件的 ADPCM 参数校准前，Typeless 仍使用它在“设置 → 音频”中选择的 Mac、耳机或 USB 麦克风
