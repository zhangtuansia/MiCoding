...
| 设备首页 | 关闭体验推荐 | 推荐卡保持隐藏，设备自动居中；偏好在重启后保留 | `home-recommendation-hidden` |
| 探索中心 | 首页推荐卡“探索” | 对齐原版一大两小卡片结构；按键测试、Actions Ring、应用 Profile 与更多玩法均进入对应真实功能 | `explore-center` |
...
| 智能操作 | 顶栏 AI 工作流入口 | 星光按钮展示 MiCoding 本地工作流说明；主操作直接进入 AI 分类模板，不保留罗技品牌停用公告或无响应链接 | `home-ai-prompt-notice-dark`、`smart-actions-ai-entry` |
| 智能操作 | 管理为空 | 对齐原版管理提示、筛选行及键盘与鼠标空态插画，不再把模板误当作已安装动作 | `smart-actions-empty`、`smart-actions-empty-dark` |
